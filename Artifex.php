...
class Artifex {
    ...
    public function __construct() {
        ...
        $this->prvt_DBNAME = "mate3rafsh-trials"; // Put your database name here 
    ...
    public function find_order_desc($table_name, $key, $value, $orderby) {
        $sql_q = "SELECT * FROM `" . strtolower($table_name) . "` WHERE `" . strtoupper($key) . "` = '" . $value . "' ORDER BY `" . strtoupper($orderby) . "` DESC";
        return $this->execute($sql_q);
    }
    ...
    public function find_and($table_name, $key, $value, $key2, $value2) { // Added By @LobnaElbakry
        $sql_q = "SELECT * FROM " . strtolower($table_name) . " WHERE " . $key . " = '" . $value . "' AND " . $key2 . " = '" . $value2 . "'";
        $reply = $this->execute($sql_q);
        return $reply;
    }
